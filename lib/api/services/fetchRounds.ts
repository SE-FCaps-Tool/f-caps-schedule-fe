...
function normalizeRoundListItem(value: unknown): RoundListItem {
  ...
  return {
    ...
    registrationDeadline: asNullableString(pick(record, "registrationDeadline", "registration_deadline")),
  };
}
...
function normalizeRoundDetail(value: unknown): RoundDetail {
  ...
  return {
    ...
    durationMinutes: durationMinutes(record),
    reviewerCount: asNumber(pick(record, "reviewerCount", "reviewer_count")),
    maxGroupsPerTimeslot: asNumber(pick(record, "maxGroupsPerTimeslot", "max_groups_per_timeslot")),
    registrationDeadline: asNullableString(pick(record, "registrationDeadline", "registration_deadline")),
    groupSelectionMode: asBoolean(pick(record, "groupSelectionMode", "group_selection_mode")),
    groupPreferenceDeadline: asNullableString(pick(record, "groupPreferenceDeadline", "group_preference_deadline")),
    registrationPhase: pick(record, "registrationPhase", "registration_phase") as RegistrationPhase | undefined,
    resultOwnerMode: asBoolean(pick(record, "resultOwnerMode", "result_owner_mode")),
...
function normalizeInvitationStatus(value: unknown): RoundInvitationStatus {
  ...
  if (value === "ACCEPTED" || value === "DECLINED" || value === "EXPIRED" || value === "WITHDRAWN") return value;
  return "PENDING";
}

function normalizeInvitation(value: unknown): RoundInvitation {
  const record = isRecord(value) ? value : {};
  const lecturerRecord = isRecord(pick(record, "lecturer")) ? (pick(record, "lecturer") as ApiRecord) : {};
  const lecturerId = asString(pick(lecturerRecord, "id", "lecturer_id") ?? pick(record, "lecturer_id"));

  return {
    id: asString(pick(record, "id", "invitation_id"), lecturerId),
    lecturer: {
      id: lecturerId,
      code: asString(pick(lecturerRecord, "code", "lecturer_code") ?? pick(record, "lecturer_code")),
      fullName: asString(pick(lecturerRecord, "fullName", "full_name", "display_name") ?? pick(record, "display_name")),
    },
    status: normalizeInvitationStatus(pick(record, "status", "invitation_status")),
    availabilitySlotCount: asNumber(pick(record, "availabilitySlotCount", "available_slot_count")),
...
function normalizeEligibleProject(value: unknown): EligibleProjectRow {
  const record = isRecord(value) ? value : {};
  const checks = isRecord(pick(record, "checks")) ? (pick(record, "checks") as ApiRecord) : {};
  ...
  return {
    ...
    checks: {
      ...
      hasActiveLeader: asBoolean(pick(checks, "hasActiveLeader", "has_active_leader")),
      hasMainSupervisor: asBoolean(pick(checks, "hasMainSupervisor", "has_main_supervisor")),
      progressionAllowed: asBoolean(pick(checks, "progressionAllowed", "progression_allowed")),
    },
    blockingReasons: Array.isArray(blockingReasons) ? blockingReasons.map((reason) => asString(reason)) : [],
    warnings: Array.isArray(warnings)
      ? warnings.map((warning) => {
          const warningRecord = isRecord(warning) ? warning : {};
          return {
            code: asString(pick(warningRecord, "code")),
            message: asString(pick(warningRecord, "message"), asString(warning)),
          };
        })
      : [],
  };
...
function normalizeRegistrationSummary(value: unknown): RegistrationSummary {
  const record = isRecord(value) ? value : {};
  const lecturers = isRecord(pick(record, "lecturers")) ? (pick(record, "lecturers") as ApiRecord) : {};
  const groups = isRecord(pick(record, "groups")) ? (pick(record, "groups") as ApiRecord) : {};

  return {
    lecturers: {
      invited: asNumber(pick(lecturers, "invited") ?? pick(record, "invited")),
      accepted: asNumber(pick(lecturers, "accepted") ?? pick(record, "responded")),
      availabilitySubmitted: asNumber(
        pick(lecturers, "availabilitySubmitted", "availability_submitted") ?? pick(record, "lecturer_availability")
      ),
      missing: asNumber(pick(lecturers, "missing")),
    },
    groups: {
      eligible: asNumber(pick(groups, "eligible")),
      preferenceSubmitted: asNumber(pick(groups, "preferenceSubmitted", "preference_submitted")),
...
export const fetchRounds = {
  ...
  list: async (
    ...
    const metaRecord = isRecord(payload) && isRecord(payload.meta) ? payload.meta : undefined;
  ...
  invitations: async (roundId: string): Promise<RoundInvitation[]> => {
    const response = await apiService.get<unknown>(`api/v1/rounds/${roundId}/invitations`);
  ...
  eligibleProjects: async (roundId: string): Promise<EligibleProjectRow[]> => {
    const response = await apiService.get<unknown>(`api/v1/rounds/${roundId}/eligible-projects`);
  ...
  attachResources: async (roundId: string, payload: AttachRoundResourcesPayload): Promise<void> => {
    const toLegacyId = (value: string) => {
      const raw = value.includes("_") ? value.slice(value.indexOf("_") + 1) : value;
      const id = Number(raw);
      if (!Number.isInteger(id) || id <= 0) throw new Error("Identifier must be numeric");
      return id;
    };
  ...
  registrationSummary: async (roundId: string): Promise<RegistrationSummary> => {
    const response = await apiService.get<unknown>(`api/v1/rounds/${roundId}/registration-summary`);
