#!/usr/bin/env node
// Sinh `lib/api/generated/schema.d.ts` từ đặc tả camelCase của BE (Phase 5 của
// plans/260823-1441-api-camelcase-convention). Copy spec vào repo trước khi generate,
// để build FE không phụ thuộc mạng hay checkout BE sau khi đã commit kết quả.
...
const defaultSpecPath = path.resolve(root, "../f-caps-schedule-be/apps/api/openapi.camel.json");
...
const localSpecPath = path.join(genDir, "openapi.camel.json");
...
if (wireCase !== "camelCase") {
  throw new Error(`Spec nguồn không phải camelCase (x-be-wire-case=${wireCase}) — dùng nhầm openapi.json?`);
}
...
const header = `/**\n * TỰ SINH — không sửa tay. Sinh lại: npm run typegen\n * Nguồn: BE commit ${beCommit} (openapi.camel.json, wire case ${wireCase})\n */\n`;
