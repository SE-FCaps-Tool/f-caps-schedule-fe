...
## Round 2: fixes from testing against the real user file (`DanhSachDeTai_FA26.xlsx`)

User reported "212 dòng bị bỏ qua" importing their actual file. Investigation (with the file copied into the BE container and inspected directly via openpyxl) found:

1. **Root cause of the reported bug:** the file has a title banner row ("PROJECTS INFORMATION") in row 1, real header in row 2. The deferred M5 risk above ("confirming the header is on row 1") turned out to be a real, live bug — `_workbook_rows` always assumed row 1, so every row misparsed. **Fixed** by adding `_project_import_rows`, a dedicated reader (not touching the shared `_workbook_rows`/`import_groups`) that scans for the row containing both "madetai" and "manhom" headers instead of assuming row 1. Also carries the real Excel row number per data row (incidentally fixes the deferred M4 item above, at no extra cost since this parser was written fresh).
2. **New finding contradicting the original "always SE" decision:** the real file's Department column has 3 distinct values (CF=93, SE=92, ITS=14) plus 12 blanks — not just SE. Presented to the user with evidence; **decision changed**: read Department per row, auto-create a `majors` row if the code doesn't exist, default to SE only when the column is blank/missing.
3. Verified end-to-end against the real file via actual HTTP calls (`TestClient` + `X-Test-Session`, not just SQL): first run → 197 created, 14 skipped (all 14 confirmed to have a genuinely blank Mã nhóm cell — correct rejection, not a bug); second run (after the Department fix) → 0 created, 197 updated, majors correctly redistributed (`CF: 83, ITS: 13, SE: 101` in the DB, matching source data).

**Second code-review pass found 2 more High-severity issues in this round's own fix, both fixed:**

- **High (fixed):** the major auto-create `INSERT` sat outside any savepoint with no `ON CONFLICT` guard — a concurrent import creating the same new department code, or a department code over 32 chars (the real DB column limit, unvalidated), would 500 the *entire* request and roll back every row already imported in that call. Fixed: moved inside the per-row `db.begin_nested()` savepoint, changed to `INSERT ... ON CONFLICT (code) DO UPDATE ... RETURNING id` (safe under contention), added a 32-char row-level validation error instead of a raw DB exception.
- **High (fixed):** the project upsert's `major_id = EXCLUDED.major_id` was unconditional — since the FE's own documented `TEMPLATE_COLUMNS` didn't even list a Department column, a sheet built from that template would import with every row defaulting to SE and **silently reset any existing CF/ITS project back to SE** on every re-import, with no error surfaced. Fixed: `major_id = CASE WHEN :dept_provided THEN EXCLUDED.major_id ELSE projects.major_id END` — blank/absent Department column now preserves the existing major instead of overwriting it, matching the same "blank doesn't wipe" policy already applied to titles. Added the Department column to the FE template guide (Phase 2) so it's discoverable.
- Also applied from that review: moved major resolution to after the GVHD lookups (a row that ultimately fails GVHD matching no longer creates an orphan major first); added the same 5MB/2000-row caps to `_project_import_rows` that the sibling `_lecturer_import_rows` already has; added a zero-sheet guard.
- 3 more regression tests added (`test_import_projects_maps_department_to_major_and_creates_new_ones` fixed to reuse a stable `ZZTEST` major code instead of leaking a new majors row per test run; `test_import_projects_rerun_with_changed_department_updates_major`; `test_import_projects_rerun_with_blank_department_keeps_existing_major` — this last one is the regression test for the second High finding above). Full suite: 9/9 passing.
